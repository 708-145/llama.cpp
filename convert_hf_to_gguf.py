...
@Model.register("granitemoehybrid")
class GraniteMoeHybridModel(Model):
    model_arch = gguf.MODEL_ARCH.GRANITEMOEHYBRID

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # self.rope_type = RopeScalingType.NONE # Or appropriate if fixed

    def set_gguf_parameters(self):
        super().set_gguf_parameters()
        hf_config = self.hf_config
        LLM_KV = gguf.LLM_KV # Alias for convenience

        # Architecture specific parameters
        self.gguf_writer.add_uint32(LLM_KV(self.model_arch, "granitemoehybrid.shared_intermediate_size"), hf_config["shared_intermediate_size"])

        layer_types = hf_config.get("layer_types", [])
        if not layer_types and "num_hidden_layers" in hf_config : # Fallback to all attention if not specified
            logger.warning("No layer_types found in config, defaulting all layers to attention.")
            layer_types = ["attention"] * hf_config["num_hidden_layers"]
        elif not layer_types:
             raise ValueError("Missing 'layer_types' and 'num_hidden_layers' in HuggingFace config, cannot determine layer structure.")


        for i, layer_type_str in enumerate(layer_types):
            layer_type_int = 0 # Default to attention
            if layer_type_str == "mamba":
                layer_type_int = 1
            elif layer_type_str == "attention":
                layer_type_int = 0
            else:
                raise ValueError(f"Unknown layer type: {layer_type_str} in GraniteMoeHybrid config")

            key = f"{gguf.MODEL_ARCH_NAMES[self.model_arch]}.layer_type.{i}"
            self.gguf_writer.add_uint32(key, layer_type_int)

        # Mamba specific hparams
        if any(lt == "mamba" for lt in layer_types):
            self.gguf_writer.add_uint32(LLM_KV(self.model_arch, "ssm.conv_kernel"), hf_config.get("mamba_d_conv", 4))
            d_inner = hf_config.get("mamba_expand", 2) * hf_config["hidden_size"]
            self.gguf_writer.add_uint32(LLM_KV(self.model_arch, "ssm.inner_size"), d_inner)
            self.gguf_writer.add_uint32(LLM_KV(self.model_arch, "ssm.state_size"), hf_config.get("mamba_d_state", 256))
            dt_rank = hf_config.get("mamba_dt_rank", hf_config["hidden_size"] // 16)
            self.gguf_writer.add_uint32(LLM_KV(self.model_arch, "ssm.time_step_rank"), dt_rank)

        # MoE specific hparams
        self.gguf_writer.add_uint32(LLM_KV(self.model_arch, "expert_count"), hf_config.get("num_local_experts"))
        self.gguf_writer.add_uint32(LLM_KV(self.model_arch, "expert_used_count"), hf_config.get("num_experts_per_tok"))
        if "expert_feed_forward_length" in hf_config:
             self.gguf_writer.add_uint32(LLM_KV(self.model_arch, "expert_feed_forward_length"), hf_config["expert_feed_forward_length"])
        elif "intermediate_size" in hf_config :
             self.gguf_writer.add_uint32(LLM_KV(self.model_arch, "expert_feed_forward_length"), hf_config["intermediate_size"])
        else:
            # Try to calculate if possible, or raise error if critical and not found
            # For this example, let's assume it might be derived or optional, if not, an error should be raised.
            logger.warning("expert_feed_forward_length not found in HF config, and no fallback to intermediate_size. MoE expert FFN size might be incorrect.")


        if "rope_theta" in hf_config:
             self.gguf_writer.add_rope_freq_base(hf_config["rope_theta"])

        # For rope_scaling, llama.cpp expects specific KVs.
        # Example: if hf_config["rope_scaling"] = {"type": "linear", "factor": 2.0}
        # self.gguf_writer.add_rope_scaling_type(gguf.RopeScalingType.LINEAR)
        # self.gguf_writer.add_rope_scaling_factor(float(hf_config["rope_scaling"]["factor"]))
        # This needs careful mapping based on what llama.cpp expects for GraniteMoeHybrid's RoPE.
        # The base LlamaModel or Model class might handle some common RoPE scaling parameters already.
        # For now, this part is left as a TODO if specific handling is needed beyond superclass.

        if "attention_bias" in hf_config:
             # Assuming attention_bias might map to clamp_kqv or a similar GGUF KV.
             # This is an example mapping, the actual GGUF key needs to be confirmed.
             # self.gguf_writer.add_bool(LLM_KV(self.model_arch, "attention.clamp_kqv"), hf_config.get("attention_bias", False))
             pass # Placeholder: confirm GGUF KV for attention_bias if used by GraniteMoeHybrid

        if "rms_norm_eps" in hf_config:
             self.gguf_writer.add_layer_norm_rms_eps(hf_config["rms_norm_eps"])

        # Granite specific scales
        if "logits_scaling" in hf_config:
            self.gguf_writer.add_float32(LLM_KV(self.model_arch, "logit_scale"), hf_config["logits_scaling"])
        if "residual_multiplier" in hf_config:
            self.gguf_writer.add_float32(LLM_KV(self.model_arch, "residual_scale"), hf_config["residual_multiplier"])
        if "embedding_multiplier" in hf_config:
            self.gguf_writer.add_float32(LLM_KV(self.model_arch, "embedding_scale"), hf_config["embedding_multiplier"])
        if "attention_multiplier" in hf_config:
             self.gguf_writer.add_float32(LLM_KV(self.model_arch, "attention.scale"), hf_config["attention_multiplier"])
